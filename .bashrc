...
if [ "$color_prompt" = yes ]; then
    ...
    case "$PROMPT_ALTERNATIVE" in
        twoline)
          PS1=$prompt_color'┌──${debian_chroot:+($debian_chroot)──}${VIRTUAL_ENV:+(\[\033[0;1m\]$(basename $VIRTUAL_ENV)'$prompt_color')}('$info_color'\u'$prompt_symbol'\h'$prompt_color')-[\[\033[0;1m\]\w'$prompt_color'] $(if [ -d .git ]; then echo -e GIT:"('$git_color'$(git_branch)'$prompt_color')"; fi)'$prompt_color'\n'$prompt_color'└─'$info_color'\$\[\033[0m\] ';;
        oneline)
